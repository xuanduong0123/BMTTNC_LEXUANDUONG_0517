import sys
import os
# Enable Qt auto screen scale factor early to avoid the runtime warning
...
# Import trực tiếp cấu trúc lớp bạn vừa gửi
...
# =============================================================
# BƯỚC 2: KHỐI THUẬT TOÁN RSA NGẦM (Giữ nguyên của bạn)
class RSACipher:
    def __init__(self):
        pass
...
class RSA_App(QMainWindow):
    def __init__(self):
        ...
        # Địa chỉ URL kết nối API Flask của bạn
        self.api_url = "http://127.0.0.1:5000/api/rsa"
        
        # KẾT NỐI SỰ KIỆN: Khớp hoàn toàn với file Python UI của bạn
        self.ui.btn_generate_key.clicked.connect(self.generate_keys)
        self.ui.btn_encrypt.clicked.connect(self.encrypt_text)
        self.ui.btn_decrypt.clicked.connect(self.decrypt_text)
        self.ui.btn_sign.clicked.connect(self.sign_text)
    ...
    def generate_keys(self):
        try:
            ...
            if response.status_code == 200:
                self.ui.txt_info.setPlainText("Tạo cặp khóa mới thành công và đã lưu file!")
            else:
                self.ui.txt_info.setPlainText(f"Lỗi Server: {response.status_code}")
        except Exception as e:
            self.ui.txt_info.setPlainText(f"Lỗi kết nối API: {str(e)}")

    def encrypt_text(self):
        try:
            msg = self.ui.txt_plain_text.toPlainText()
            payload = {"message": msg, "key_type": "public"}
            response = requests.post(f"{self.api_url}/encrypt", json=payload)
            if response.status_code == 200:
                result = response.json()
                # Gán trực tiếp vào ô txt_cipher_text chuẩn tên giao diện của bạn
                self.ui.txt_cipher_text.setPlainText(result.get("encrypted_message", ""))
        except Exception as e:
            self.ui.txt_info.setPlainText(str(e))
    ...
    def decrypt_text(self):
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                self.ui.txt_plain_text.setPlainText(result.get("decrypted_message", ""))
        except Exception as e:
            self.ui.txt_info.setPlainText(str(e))
    ...
    def sign_text(self):
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                self.ui.txt_sign.setPlainText(result.get("signature", ""))
        except Exception as e:
            self.ui.txt_info.setPlainText(str(e))
    ...
    def verify_signature(self):
        try:
            ...
            if response.status_code == 200:
                ...
                self.ui.txt_info.setPlainText(status)
        except Exception as e:
            self.ui.txt_info.setPlainText(str(e))
